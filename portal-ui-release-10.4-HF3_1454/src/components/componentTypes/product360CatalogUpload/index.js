...
const Product360CatalogUpload = ({ component, match}) => {
    ...
    const fetchProductData = (tokenKey,selectedCatalog)=>{
         let url = `${component.serverUrl}/html/Hsp.html?embedded=true&authToken=${tokenKey}&locale=${language}#upload:catalogId=${selectedCatalog.value}`;
         setUrl(url);
    }
    ...
    return (
        <Card className="external-link-width" data-testid="product360_catalog_upload">
            <Card.Header title={component.title}>
                <Toolbar>
                    <Dropdown 
                        className="product__catalog"
                        data-testid="product360_catalog_upload_document" 
                        options={catalogs} 
...
export default Product360CatalogUpload;
