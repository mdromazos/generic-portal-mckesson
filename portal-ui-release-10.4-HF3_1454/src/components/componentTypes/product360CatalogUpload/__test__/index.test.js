...
import Product360CatalogUpload from '../index';
import { StateContext } from '../../../../context/stateContext';
import { render, cleanup } from '@testing-library/react';
import {component_data, match, catalogOptions} from '../__mocks__/p360-upload-data';
...
const renderForm = () => render(
    <StateContext.Provider value={[{
        ...
        <Product360CatalogUpload component={component_data} match={match}/>
...
describe('Rendering Portal Text component', () => {
    ...
    it("Renders Portal Text component properly", () => {
        ...
        expect(getByTestId('product360_catalog_upload')).not.toBeNull();
        expect(getByTestId('product360_catalog_upload_document')).not.toBeNull();
    });

    it("Portal Text component Heading and body should match with the data", () => {
        const { getByTestId } = renderForm();
        expect(getByTestId('product360_catalog_upload').textContent).toMatch(component_data.title);
    });
});
